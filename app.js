...
app.post("/login", (req, res) => {
    ...
    console.log("LOGIN DATA:", email, password); // 👈 مهم
...
app.post("/admin-login", (req, res) => {
    ...
    if(username === "oscar" && password === "Mostafa&123"){
...
app.post("/request", (req, res) => {
    ...
    const { email, amount, type, address } = req.body;
...
let amount = localStorage.getItem("lastAmount");
...
let req = data.find(r => r.email === user.email && r.amount == amount);
...
if(lang === "ar"){
document.querySelector(".section-title").innerText = "products";
}
if(lang === "en"){
